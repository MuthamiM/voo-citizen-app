...
class _ServicesScreenState extends State<ServicesScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      final announcementsRes = await http.get(Uri.parse('${AuthService.backendUrl}/announcements'));
      ...
      appBar: AppBar(
        ...
        leading: IconButton(
          icon: Container(
            ...
            decoration: BoxDecoration(color: Color(0xFF2A2A2A), shape: BoxShape.circle),
                        ...
                        child: Row(
                          ...
                          children: [
                            Container(
                              ...
                              child: const Row(
                                children: [
                                  Icon(Icons.verified_user_outlined, size: 14, color: primaryOrange),
                                  SizedBox(width: 4),
                                  Text('Official Services', style: TextStyle(color: primaryOrange, fontSize: 11, fontWeight: FontWeight.bold)),
                                ],
                              ),
                            ),
                          ],
                            ...
                            if (_announcements.where((a) => !StorageService.getReadAnnouncementIds().contains(a['id']?.toString() ?? '')).isEmpty)
                              Container(
                                padding: const EdgeInsets.all(24),
                                decoration: BoxDecoration(
                                  color: const Color(0xFF2A2A2A),
                              ...
                              ..._announcements.where((a) => !StorageService.getReadAnnouncementIds().contains(a['id']?.toString() ?? '')).map((a) {
                                final id = a['id']?.toString() ?? '';
                                final date = a['created_at'] != null ? DateTime.parse(a['created_at']) : DateTime.now();
                                  ...
                                  child: ExpansionTile(
                                    ...
                                    subtitle: Padding(
                                      ...
                                      child: Column(
                                          crossAxisAlignment: CrossAxisAlignment.start,
                                          children: [
                                            Text(a['content'] ?? '', maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(color: textMuted, fontSize: 13)),
                                            const SizedBox(height: 8),
                                            Text(dateStr, style: TextStyle(color: textMuted.withOpacity(0.6), fontSize: 12)),
                                          ]
                                      )
                                    ),
                                    children: [
                                      ...
                                      SizedBox(
                                        ...
                                        child: OutlinedButton.icon(
                                          onPressed: () async {
                                            await StorageService.markAnnouncementRead(id);
                                            setState(() {});
                                            ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Announcement marked as read'), behavior: SnackBarBehavior.floating, duration: Duration(seconds: 1)));
                                          },
