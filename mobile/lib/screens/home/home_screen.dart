...
class _HomeScreenState extends State<HomeScreen> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    // Check for updates and sync pending data after the first frame
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _checkForUpdates();
      DashboardService.syncPendingIssues();
    });
  }
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  SizedBox(
                    height: 250,
                    child: announcements.where((a) => !StorageService.getReadAnnouncementIds().contains(a['id']?.toString() ?? '')).isEmpty 
                      ? Center(
                          child: Container(
                            ...
                            child: const Column(
                              ...
                              children: [
                                ...
                                SizedBox(height: 8),
                                Text('No new announcements', style: TextStyle(color: textMuted)),
                              ],
                            ),
                          ),
                        )
                      : ListView.builder(
                          ...
                          itemCount: announcements.where((a) => !StorageService.getReadAnnouncementIds().contains(a['id']?.toString() ?? '')).length,
                          itemBuilder: (context, index) {
                            final filteredList = announcements.where((a) => !StorageService.getReadAnnouncementIds().contains(a['id']?.toString() ?? '')).toList();
                                ...
                                child: ExpansionTile(
                                  ...
                                  subtitle: Column(
                                    ...
                                    children: [
                                      ...
                                      Text(dateStr, style: const TextStyle(color: primaryOrange, fontSize: 11)),
                                    ],
                                  ),
                                  children: [
  ...
  Future<void> _refreshHome() async {
    if (await StorageService.isOnline()) {
      await DashboardService.syncPendingIssues();
                  ...
                  child: SingleChildScrollView(
                    child: Column(
                      children: [
                        ...
                        const SizedBox(height: 20),
                        // Version
                        Center(child: Text('Version 11.0.1', style: TextStyle(color: textMuted.withOpacity(0.5), fontSize: 12))),
                        const SizedBox(height: 100),
                      ],
                    ),
                  ),
